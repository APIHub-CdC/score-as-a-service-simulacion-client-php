...
class ResponseDatosHistoricos implements ModelInterface, ArrayAccess
{
    const DISCRIMINATOR = null;
    
    protected static $apihubModelName = 'ResponseDatosHistoricos';
    
    protected static $apihubTypes = [
        'http_status' => 'float',
        'success' => 'string',
        'message' => 'string',
        'data' => '\ScoreAsAService\Client\Model\ResponseDatosHistoricosData[]'
    ];
    ...
    public function setMessage($message)
    {
        if (!is_null($message) && (mb_strlen($message) > 20)) {
            throw new \InvalidArgumentException('invalid length for $message when calling ResponseDatosHistoricos., must be smaller than or equal to 20.');
        }
        if (!is_null($message) && (mb_strlen($message) < 4)) {
            throw new \InvalidArgumentException('invalid length for $message when calling ResponseDatosHistoricos., must be bigger than or equal to 4.');
        }
