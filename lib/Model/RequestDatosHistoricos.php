...
class RequestDatosHistoricos implements ModelInterface, ArrayAccess
{
    ...
    public function setNperscve($nperscve)
    {
        if ((mb_strlen($nperscve) > 9)) {
            throw new \InvalidArgumentException('invalid length for $nperscve when calling RequestDatosHistoricos., must be smaller than or equal to 9.');
        }
        if ((mb_strlen($nperscve) < 9)) {
            throw new \InvalidArgumentException('invalid length for $nperscve when calling RequestDatosHistoricos., must be bigger than or equal to 9.');
        }
