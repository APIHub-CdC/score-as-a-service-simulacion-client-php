...
class ResponseConsulta implements ModelInterface, ArrayAccess
{
    const DISCRIMINATOR = null;
    
    protected static $apihubModelName = 'ResponseConsulta';
    
    protected static $apihubTypes = [
        'http_status' => 'float',
        'success' => 'string',
        'message' => 'string',
        'data' => '\ScoreAsAService\Client\Model\ResponseConsultaData[]'
    ];
    ...
    public function setSuccess($success)
    {
        if (!is_null($success) && (mb_strlen($success) > 5)) {
            throw new \InvalidArgumentException('invalid length for $success when calling ResponseConsulta., must be smaller than or equal to 5.');
        }
        if (!is_null($success) && (mb_strlen($success) < 4)) {
            throw new \InvalidArgumentException('invalid length for $success when calling ResponseConsulta., must be bigger than or equal to 4.');
        }
    ...
    public function setMessage($message)
    {
        if (!is_null($message) && (mb_strlen($message) > 20)) {
            throw new \InvalidArgumentException('invalid length for $message when calling ResponseConsulta., must be smaller than or equal to 20.');
        }
        if (!is_null($message) && (mb_strlen($message) < 4)) {
            throw new \InvalidArgumentException('invalid length for $message when calling ResponseConsulta., must be bigger than or equal to 4.');
        }
