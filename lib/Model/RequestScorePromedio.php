...
class RequestScorePromedio implements ModelInterface, ArrayAccess
{
    const DISCRIMINATOR = null;
    
    protected static $apihubModelName = 'RequestScorePromedio';
    ...
    public function setNperscve($nperscve)
    {
        if ((mb_strlen($nperscve) > 9)) {
            throw new \InvalidArgumentException('invalid length for $nperscve when calling RequestScorePromedio., must be smaller than or equal to 9.');
        }
        if ((mb_strlen($nperscve) < 9)) {
            throw new \InvalidArgumentException('invalid length for $nperscve when calling RequestScorePromedio., must be bigger than or equal to 9.');
        }
