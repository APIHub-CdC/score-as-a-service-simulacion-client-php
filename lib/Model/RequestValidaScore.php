...
class RequestValidaScore implements ModelInterface, ArrayAccess
{
    const DISCRIMINATOR = null;
    
    protected static $apihubModelName = 'RequestValidaScore';
    ...
    public function setNperscve($nperscve)
    {
        if ((mb_strlen($nperscve) > 9)) {
            throw new \InvalidArgumentException('invalid length for $nperscve when calling RequestValidaScore., must be smaller than or equal to 9.');
        }
        if ((mb_strlen($nperscve) < 9)) {
            throw new \InvalidArgumentException('invalid length for $nperscve when calling RequestValidaScore., must be bigger than or equal to 9.');
        }
